...
//            System.out.println("Tradeoffer declined!");
    ...
    public double getSystemBonusFact(int base) {
        return (isBaseUnlocked(base) && base != 10) ? SYSTEM_OWNED_FACT : 1.0;
    }
    ...
    public boolean canAfford(Price price) {
        switch (price.getUnit()) {
            case MONEY:
                return price.getPrice() <= score;
            case PRESTIGE_POINTS:
                return price.getPrice() <= prestigePoints;
            default:
                throw new UnsupportedOperationException("Unknown price unit: " + price.getUnit());
        }
    ...
    public void subtractScore(Price price) {
        switch (price.getUnit()) {
            case MONEY:
                score -= price.getPrice();
                break;
            case PRESTIGE_POINTS:
                prestigePoints -= price.getPrice();
                break;
            default:
                throw new UnsupportedOperationException("Unknown price unit: " + price.getUnit());
        }
